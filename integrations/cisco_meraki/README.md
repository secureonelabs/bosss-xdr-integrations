...
1.  **Rules and Decoders Deployment**:
    * In BOSSS XDR Dashboard, navigate to `Server Management` > `Rules`.
    ...
    * In BOSSS XDR Dashboard, navigate to `Server Management` > `Decoders`.
...
2.  **Configuration in `ossec.conf`**:
    * Ensure your `ossec.conf` (on the BOSSS XDR Server or Agent receiving logs from the syslog server) is configured to include these new files. 
    ```
    <localfile>
...
3.  **Restart BOSSS XDR Cluster**: After making these configuration changes, you must restart the Wazuh cluster to apply them.
    ```bash
...
3.  **BOSSS XDR Agent Collection**: A Wazuh agent (or the Wazuh server directly if it's acting as the syslog receiver) is configured to collect these syslog events from the syslog server (either by listening on a UDP/TCP port or by monitoring a log file).
4.  **BOSSS XDR Server Processing**: The collected events are forwarded to the Wazuh server, where the custom Meraki decoders parse the raw syslog messages, and the custom Meraki rules generate alerts based on the decoded information.
5.  **BOSSS XDR Dashboard Visualization**: Alerts and events are then available for search, observation, and visualization in the BOSSS XDR Dashboard.
...
3.  **Check BOSSS XDR Agent/Server Logs**:
    ...
    * On the BOSSS XDR Server, check `/var/ossec/logs/ossec.log` for processed events and alerts. Look for alerts generated by your custom Meraki rules.
    * Example log snippet you might look for (actual content will vary):
        ...
        ** Alert 1234567890.12345 **: - meraki,
        ...
        Level: 5
        ```
4.  **View in BOSSS XDR Dashboard**:
    * Log in to your BOSSS XDR Dashboard.
...
* [BOSSS XDR Documentation: Forward syslog events](https://documentation.wazuh.com/current/user-manual/reference/ossec-conf/localfile.html#localfile-syslog)
